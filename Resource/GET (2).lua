...
-- Custom UI/UX global override registries
Creator.CornerRegistry = Creator.CornerRegistry or {}
Creator.FontRegistry = Creator.FontRegistry or {}
Creator.CustomUX = Creator.CustomUX or {
	CornerRadius = nil,   -- number (px) or nil = use original
	Font = nil,           -- Enum.Font or nil = use original
	AccentColor = nil,    -- Color3 or nil = use theme accent
	BackgroundColor = nil,-- Color3 or nil = use theme AcrylicMain
	BackgroundTransparency = nil, -- number or nil
}

function Creator.ApplyCustomUX()
	local UX = Creator.CustomUX

	-- Corner radius override
	for Obj, OriginalRadius in next, Creator.CornerRegistry do
		if Obj and Obj.Parent then
			if UX.CornerRadius ~= nil then
				Obj.CornerRadius = UDim.new(0, UX.CornerRadius)
			else
				Obj.CornerRadius = OriginalRadius
			end
		else
			Creator.CornerRegistry[Obj] = nil
		end
	end

	-- Font override
	for Obj, Original in next, Creator.FontRegistry do
		if Obj and Obj.Parent then
			if UX.Font ~= nil then
				pcall(function() Obj.FontFace = UX.Font end)
			else
				pcall(function() Obj.Font = Original.Font end)
				pcall(function() Obj.FontFace = Original.FontFace end)
			end
		else
			Creator.FontRegistry[Obj] = nil
		end
	end

	-- Accent color override
	if UX.AccentColor ~= nil then
		Themes[Library.Theme].Accent = UX.AccentColor
	end

	-- Background color / transparency override
	if Library.Window then
		local Acrylic = Library.Window.AcrylicPaint
		if Acrylic and Acrylic.Frame then
			if UX.BackgroundColor ~= nil then
				Acrylic.Frame.BackgroundColor3 = UX.BackgroundColor
			end
			if UX.BackgroundTransparency ~= nil then
				Acrylic.Frame.BackgroundTransparency = UX.BackgroundTransparency
			end
		end
	end
...
function Creator.New(Name, Properties, Children)
	...
	-- Register for global Custom UI/UX overrides
	if Name == "UICorner" then
		Creator.CornerRegistry[Object] = Object.CornerRadius
		if Creator.CustomUX.CornerRadius ~= nil then
			Object.CornerRadius = UDim.new(0, Creator.CustomUX.CornerRadius)
		end
	elseif Name == "TextLabel" or Name == "TextButton" or Name == "TextBox" then
		Creator.FontRegistry[Object] = { Font = Object.Font, FontFace = Object.FontFace }
		if Creator.CustomUX.Font ~= nil then
			pcall(function() Object.FontFace = Creator.CustomUX.Font end)
		end
	end
...
local InterfaceManager = {} do
	...
	InterfaceManager.Settings = {
		...
		Transparency = true,
		MenuKeybind = "M",
		CustomCornerEnabled = false,
		CustomCornerRadius = 8,
		CustomFontEnabled = false,
		CustomFontName = "Gotham",
		CustomAccentEnabled = false,
		CustomAccentColor = "#3399FF",
		CustomBackgroundEnabled = false,
		CustomBackgroundColor = "#1E1E1E",
		CustomBackgroundTransparency = 0,
	}
	...
	InterfaceManager.FontNames = { "Gotham", "Gotham Bold", "Roboto", "Roboto Mono", "SourceSans", "Montserrat" }

	function InterfaceManager:ApplyCustomUX()
		local Settings = InterfaceManager.Settings

		Creator.CustomUX.CornerRadius = Settings.CustomCornerEnabled and Settings.CustomCornerRadius or nil
		Creator.CustomUX.Font = Settings.CustomFontEnabled and InterfaceManager.FontMap[Settings.CustomFontName] or nil
		Creator.CustomUX.AccentColor = Settings.CustomAccentEnabled and Color3.fromHex(Settings.CustomAccentColor) or nil
		Creator.CustomUX.BackgroundColor = Settings.CustomBackgroundEnabled and Color3.fromHex(Settings.CustomBackgroundColor) or nil
		Creator.CustomUX.BackgroundTransparency = Settings.CustomBackgroundEnabled and Settings.CustomBackgroundTransparency or nil

		Creator.ApplyCustomUX()
	end
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		local CustomSection = tab:AddSection("Custom UI/UX")

		local CornerToggle = CustomSection:AddToggle("CustomCornerToggle", {
			Title = "Custom Corner Radius",
			Description = "Bo tròn các khối UI theo giá trị tuỳ chỉnh.",
			Default = Settings.CustomCornerEnabled,
			Callback = function(Value)
				Settings.CustomCornerEnabled = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local CornerSlider = CustomSection:AddSlider("CustomCornerRadius", {
			Title = "Corner Radius",
			Description = "Độ bo tròn (px).",
			Default = Settings.CustomCornerRadius,
			Min = 0,
			Max = 24,
			Rounding = 0,
			Callback = function(Value)
				Settings.CustomCornerRadius = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local FontDropdown = CustomSection:AddDropdown("CustomFontName", {
			Title = "Custom Font",
			Description = "Chọn font chữ tuỳ chỉnh.",
			Values = InterfaceManager.FontNames,
			Default = Settings.CustomFontName,
			Callback = function(Value)
				Settings.CustomFontName = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local FontToggle = CustomSection:AddToggle("CustomFontToggle", {
			Title = "Apply Custom Font",
			Description = "Bật để áp dụng font tuỳ chỉnh cho toàn bộ UI.",
			Default = Settings.CustomFontEnabled,
			Callback = function(Value)
				Settings.CustomFontEnabled = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local AccentToggle = CustomSection:AddToggle("CustomAccentToggle", {
			Title = "Custom Accent Color",
			Description = "Đổi màu nhấn (accent) của toàn bộ UI.",
			Default = Settings.CustomAccentEnabled,
			Callback = function(Value)
				Settings.CustomAccentEnabled = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local AccentPicker = CustomSection:AddColorPicker("CustomAccentColor", {
			Title = "Accent Color",
			Description = "Chọn màu accent.",
			Default = Color3.fromHex(Settings.CustomAccentColor),
			Callback = function(Color)
				Settings.CustomAccentColor = Color:ToHex()
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local BackgroundToggle = CustomSection:AddToggle("CustomBackgroundToggle", {
			Title = "Custom Background",
			Description = "Đổi màu/độ trong suốt của background chính.",
			Default = Settings.CustomBackgroundEnabled,
			Callback = function(Value)
				Settings.CustomBackgroundEnabled = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local BackgroundPicker = CustomSection:AddColorPicker("CustomBackgroundColor", {
			Title = "Background Color",
			Description = "Chọn màu nền chính.",
			Default = Color3.fromHex(Settings.CustomBackgroundColor),
			Transparency = Settings.CustomBackgroundTransparency,
			Callback = function(Color)
				Settings.CustomBackgroundColor = Color:ToHex()
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local BackgroundTransSlider = CustomSection:AddSlider("CustomBackgroundTransparency", {
			Title = "Background Transparency",
			Description = "Độ trong suốt của background (0 = đặc, 1 = trong suốt hoàn toàn).",
			Default = Settings.CustomBackgroundTransparency,
			Min = 0,
			Max = 1,
			Rounding = 2,
			Callback = function(Value)
				Settings.CustomBackgroundTransparency = Value
				InterfaceManager:ApplyCustomUX()
				InterfaceManager:SaveSettings()
			end
		})

		local ResetButton = CustomSection:AddButton({
			Title = "Reset Custom UI/UX",
			Description = "Khôi phục giao diện về mặc định của theme.",
			Callback = function()
				Settings.CustomCornerEnabled = false
				Settings.CustomFontEnabled = false
				Settings.CustomAccentEnabled = false
				Settings.CustomBackgroundEnabled = false

				CornerToggle:SetValue(false)
				FontToggle:SetValue(false)
				AccentToggle:SetValue(false)
				BackgroundToggle:SetValue(false)
